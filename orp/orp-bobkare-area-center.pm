...
sub find_area_center {
    ...
    for ( my $i = 0 ; $i < $nodecount ; $i++ ) {
        my $from =
          ( $i == 0 ) ? $nodes->[ $nodecount - 1 ] : $nodes->[ $i - 1 ];
        my $through = $nodes->[$i];
        ...
        my $from_angle =
          atan2( $from_pt->[1] - $vertex->[1], $from_pt->[0] - $vertex->[0] ) +
          pi;
        my $to_angle =
          atan2( $to_pt->[1] - $vertex->[1], $to_pt->[0] - $vertex->[0] ) + pi;
        ...
        for ( my $j = 0 ; $j < $nodecount ; $j++ ) {
            ...
            my $segment_end =
              ( $j == $nodecount - 1 ) ? $nodes->[0] : $nodes->[ $j + 1 ];
...
#debug(sprintf("  other segment %8.5f,%8.5f - %8.5f,%8.5f", @$start_pt, @$end_pt));
            ...
            my $intersection =
              get_line_intersection( $vertex, $linepoint, $start_pt, $end_pt );
