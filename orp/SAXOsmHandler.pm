...
sub start_element {
    ...
    elsif ( ( $element->{Name} eq 'nd' ) and ( ref $self->{current} eq 'way' ) )
    {
        ...
        and ( ref $self->{current} eq 'relation' ) )
    {
        ...
        push(
            ...
            [
                ...
                $element->{Attributes}{type} . ':' . $element->{Attributes}{ref}
            ]
        );
    }
  ...
  # also extract layer information into a direct hash member for ease of access.
        ...
        $self->{current}{tags}{ $element->{Attributes}{k} } =
        ...
        $b->{minlat} = $minlat
          if !defined( $b->{minlat} ) || $minlat < $b->{minlat};
        $b->{minlon} = $minlon
          if !defined( $b->{minlon} ) || $minlon < $b->{minlon};
        $b->{maxlat} = $maxlat
          if !defined( $b->{maxlat} ) || $maxlat > $b->{maxlat};
        $b->{maxlon} = $maxlon
          if !defined( $b->{maxlon} ) || $maxlon > $b->{maxlon};
    }
    else {
