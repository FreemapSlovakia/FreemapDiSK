...
sub start_element {
    ...
    if ( $element->{Name} eq 'node' ) {
        ...
        $node_storage->{$id} = $current_element = {
            'id'        => $id,
            'layer'     => 0,
        ...
        return if ( $element->{'Attributes'}->{'action'} eq 'delete' );
        my $id = $element->{'Attributes'}->{'id'};
        $way_storage->{$id} = $current_element = {
            'id'        => $id,
            'layer'     => 0,
            'nodes'     => [],
            'relations' => []
        ...
        return if ( $element->{'Attributes'}->{'action'} eq 'delete' );
        my $id = $element->{'Attributes'}->{'id'};
        $relation_storage->{$id} = $current_element = {
            'id'        => $id,
            'members'   => [],
            'relations' => []
        };
        ...
        and ( ref($current_element) eq 'way' ) )
    {
        push(
            @{ $current_element->{'nodes'} },
            $node_storage->{ $element->{'Attributes'}->{'ref'} }
        );
    }
    elsif ( ( $element->{Name} eq 'member' )
        and ( ref($current_element) eq 'relation' ) )
    {
  ...
  # also extract layer information into a direct hash member for ease of access.
        $current_element->{"tags"}->{ $element->{"Attributes"}->{"k"} } =
          $element->{"Attributes"}->{"v"};
        $current_element->{"layer"} = $element->{"Attributes"}->{"v"}
