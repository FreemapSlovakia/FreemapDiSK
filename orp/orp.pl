...
my $handler =
  SAXOsmHandler->new( $node_storage, $way_storage, $relation_storage,
    \%data_bounds );
...
if ($bbox) {
    ...
    (
        $opt_bounds{minlat}, $opt_bounds{minlon},
        $opt_bounds{maxlat}, $opt_bounds{maxlon}
    ) = split( /,/, $bbox );
}

my $rules = XML::XPath->new( filename => $rule_file );
my $data = get_variable( "data/file", "" );
...
sub assemble_closed_ways {
    ...
    while ( @$inputways > 0 ) {
        ...
        my $relations =
          [ @{ $way->{'relations'} } ]
          ;    # TODO: Make sure no duplicate entries are present
        ...
        while ( @$inputways > 0 && $found ) {
            ...
            foreach my $index ( 0 .. $#{$inputways} ) {
                    ...
                    && ( $nodes->[-1] eq $sorted[0] ) )
                {
                    ...
                    # Add tags to taglist
                    while ( my ( $key, $val ) =
                        each( %{ @$inputways[$index]->{'tags'} } ) )
                    {
                        $tags->{$key} = $val;
                    }
                    push( @$relations,
                        @{ @$inputways[$index]->{'relations'} } );
        ...
        debug("Unclosed way in multipolygon $relation->{'id'}")
          if (  defined @$nodes[0]
            and @$nodes[0] ne @$nodes[-1]
            and $debug->{'multipolygon'} );
    }
    return $outputways;
}
...
sub tags_subset {
    ...
    foreach my $tag ( keys %{ $first->{"tags"} } ) {
        next
          if ( $tag =~
/^(name|created_by|note|layer|osmarender:areaCenterLat|osmarender:areaCenterLon|osmarender:areaSize)$/
          );
        return 0
          unless defined( $second->{'tags'}{$tag} )
              && $first->{'tags'}{$tag} eq $second->{'tags'}{$tag};
    }
    return 1;
}
...
sub multipolygon_has_tags {
    ...
    foreach my $key ( keys %{ $multipolygon->{'tags'} } ) {
        if ( $key !~
            m/^(type|created_by|fixme|name.*|note.*|source.*|osmarender.*)$/ )
        {
            return 1;
        }
    }
...
foreach my $rel ( values(%$relation_storage) ) {
    next
      unless defined $rel->{'tags'}->{'type'}
          and $rel->{'tags'}->{'type'} eq 'multipolygon';
    my $outerways = ();
    my $innerways = ();
    $multipolygon_wayid += 1;
    my $multipolygon = {
    ...
    foreach my $relmember ( @{ $rel->{"members"} } ) {
        ...
        if ( $role eq "outer" ) {
            ...
            debug(
                "Unknown role \"$role\" in multipolygon relation $rel->{'id'}!")
              if ( $debug->{'multipolygon'} );
        }
        $obj->{'multipolygon'} =
          1;    # Mark object as beeing part of a multipolygon
    }
    ...
    if ( defined(@$outerways) ) {
        if ( !multipolygon_has_tags($multipolygon) ) {
            ...
            if ( !multipolygon_has_tags($multipolygon) ) {
                ...
                debug(
"No common tags on outer ways in multipolygon relation $rel->{'id'}!"
                ) if $debug->{'multipolygon'};
            }
            else {
                ...
                # just a warning
                debug(
"Copied common tags from outer way(s) into multipolygon relation $rel->{'id'}!"
                ) if $debug->{'multipolygon'};
            }
        }
    }
  ...
  WAY:
    foreach my $way ( @{ $multipolygon->{'inner'} } ) {
        ...
        foreach ( @{ $way->{'relations'} } ) {
            ...
            next WAY
              if (  $role eq 'outer'
                and defined $wayrelation->{"tags"}->{"type"}
...
my $marginaliaTopHeight =
  ( $title ne "" ) ? 40 : ( $showBorder eq "yes" ) ? 1.5 : 0;
my $marginaliaBottomHeight =
    ( $showScale eq "yes" or $showLicense eq "yes" ) ? 45
  : ( $showBorder eq "yes" ) ? 1.5
  :                            0;
...
our $dataHeight =
  ( ProjectF($maxlat) - ProjectF($minlat) ) * 180 / pi * 10000 * $scale;
our $minimumMapWidth  = get_variable( "minimumMapWidth",  undef );
our $minimumMapHeight = get_variable( "minimumMapHeight", undef );
our $documentWidth =
  ( $dataWidth > $minimumMapWidth * $km ) ? $dataWidth : $minimumMapWidth * $km;
our $documentHeight =
  ( $dataHeight > $minimumMapHeight * $km )
  ? $dataHeight
  : $minimumMapHeight * $km;
...
my $svgHeight =
  $documentHeight + $marginaliaBottomHeight + $marginaliaTopHeight;
...
$writer->raw(
    $rules->findnodes_as_string(
        "//rules/defs/*[local-name() != 'svg' and local-name() != 'symbol']")
);
$writer->endTag("defs");
...
foreach my $node ( $rules->find('//rules/defs/svg:svg')->get_nodelist ) {
    ...
    my %attributes =
      map { $_->getName => $_->getNodeValue } $node->getAttributes;
    $writer->startTag( "symbol", %attributes );
    $writer->raw(
        $rules->findnodes_as_string("//rules/defs/svg:svg[\@id='$id']/*") );
...
# not found
    ...
    my %refs =
      map { $_, 1 }
      map { $_->getNodeValue }
      $rules->find('/rules//symbol/@ref | /rules//areaSymbol/@ref')
      ->get_nodelist;
    foreach my $file ( keys %refs ) {
        if ( not exists $symbols{ 'symbol-' . $file } ) {
            my $symbolFile =
              XML::XPath->new( filename => $symbolsDir . "/" . $file . ".svg" );
            ...
            if ( $symbol->size() == 1 ) {
                ...
                my %namespaces =
                  map { "xmlns:" . $_->getPrefix => $_->getExpanded }
                  $svgNode->getNamespaces;
                $namespaces{'xmlns'} = $namespaces{'xmlns:#default'};
                delete $namespaces{'xmlns:#default'};
                my %attributes =
                  map { $_->getName => $_->getValue } $svgNode->getAttributes;
...
foreach my $include ( $rules->find("/rules//include")->get_nodelist ) {
    my $includeFile = XML::XPath->new(
        filename => File::Spec->catdir(
            $Bin, '../stylesheets/', $include->getAttribute("ref")
        )
    );
...
if ( get_variable( "withOSMLayers", "yes" ) eq "no" ) {
    ...
    render_layer(
        ...
        [
            grep( {
                    ( $_->{'instruction'}->getAttribute('z-mode') || 'normal' )
                      eq 'bottom'
                } @$drawing_commands )
        ]
    );
    ...
    foreach my $command (@$drawing_commands) {
        ...
        next
          unless ( $instruction->getAttribute('z-mode') || 'normal' ) eq
          'normal';
        ...
        foreach my $element ( @{ $command->{'elements'} } ) {
            ...
            if ( !defined( $normalInstructions{$elementLayer} ) ) {
                $normalInstructions{$elementLayer} =
                  [ { 'instruction' => $instruction } ];
            }
            if ( $normalInstructions{$elementLayer}[-1]->{'instruction'} !=
                $instruction )
            {
                push @{ $normalInstructions{$elementLayer} },
                  { 'instruction' => $instruction };
            }
            push @{ $normalInstructions{$elementLayer}[-1]->{'elements'} },
              $element;
        }
    }
    ...
    render_layer(
        'top',
        [
            grep( ( $_->{'instruction'}->getAttribute('z-mode') || 'normal' ) eq
                  'top',
                @$drawing_commands )
        ]
    );
...
draw_map_decoration();
draw_marginalia()
  if ( $title ne "" || $showScale eq "yes" || $showLicense eq "yes" );
...
sub render_layer {
    ...
    foreach my $command (
        sort {
            ( $a->{'instruction'}->getAttribute('z-index') || 0 )
              <=> ( $b->{'instruction'}->getAttribute('z-index') || 0 )
        } @$commands
      )
    {
        $instructions{ $command->{'instruction'}->getName() }->{'func'}
          ->( $command->{'instruction'}, undef, $command->{'elements'} );
    }
...
sub generate_paths {
    ...
    foreach my $way_id ( keys %referenced_ways ) {
        ...
        debug('ERROR: Multipolygon in generate_paths!')
          if ref $way eq 'multipolygon' and $debug->{'multipolygon'};
        my $types  = $referenced_ways{$way_id};
        my $tags   = $way->{"tags"};
        my $points = [];
        foreach ( @{ $way->{"nodes"} } ) {
            push( @$points, [ $_->{"lat"}, $_->{"lon"} ] )
...
# suggested to use this feature for tiles@home.
        if ( $types->{'normal_extended'} or $types->{'reverse_extended'} ) {
            ...
            for ( my $i = 1 ; $i < scalar(@$points) ; $i++ ) {
                my $pwl =
                  sqrt( ( $points->[$i]->[0] - $points->[ $i - 1 ]->[0] )**2 +
            ...
            my $endnode =
              [ $points->[$n]->[0] + $aa, $points->[$n]->[1] + $bb ];

            foreach my $t ( "normal_extended", "reverse_extended" ) {
                $writer->emptyTag(
                    "path",
                    "id" => "way_${t}_$way_id",
                    "d"  => ( $t eq "normal_extended" )
                    ? make_path( $startnode, @$points,          $endnode )
                    : make_path( $endnode,   reverse(@$points), $startnode )
                );
            }
        }
        ...
        if ( $types->{'mid'} ) {
            $writer->emptyTag(
                "path",
                "id" => "way_mid_$way_id",
                "d"  => make_path( $midpoint_head, @$points, $midpoint_tail )
            ) if scalar(@$points);
        }
    }
    $writer->endTag("defs");
    ...
    if ( get_variable("showGrid") eq "yes" ) {
        ...
        for ( my $i = 1 ; $i < $documentHeight / $gridSpacingPx ; $i++ ) {
            $writer->emptyTag(
                ...
                'id'    => 'grid-hori-' . $i,
                'x1'    => '0px',
                'y1'    => sprintf( '%fpx', $i * $gridSpacingPx ),
                'x2'    => $documentWidth . 'px',
                'y2'    => sprintf( '%fpx', $i * $gridSpacingPx ),
                'class' => 'map-grid-line'
            );
        }
        ...
        for ( my $i = 1 ; $i < $documentWidth / $gridSpacingPx ; $i++ ) {
            $writer->emptyTag(
                'line',
                'id'    => 'grid-vert-' . $i,
                'x1'    => sprintf( '%fpx', $i * $gridSpacingPx ),
                'y1'    => 0,
                'x2'    => sprintf( '%fpx', $i * $gridSpacingPx ),
                'y2'    => $documentHeight . 'px',
                'class' => 'map-grid-line'
            );
        }
        $writer->endTag('g');
...
sub draw_marginalia {
    ...
    if ( $title ne "" ) {
        ...
        $writer->emptyTag(
            ...
            y        => '0px',
            'width'  => $documentWidth . 'px',
            'height' => sprintf( '%fpx', $marginaliaTopHeight - 5 )
        );
        $writer->dataElement(
            'text', $title,
    ...
    if ( $showScale eq "yes" || $showLicense eq "yes" ) {
        ...
        $writer->emptyTag(
            ...
            'x'      => '0px',
            y        => sprintf( '%fpx', $documentHeight + 5 ),
            'height' => '40px',
            width    => $documentWidth . 'px',
        ...
        if ( $showLicense eq 'yes' ) {
            $writer->startTag(
                ...
                'transform'          => sprintf(
                    'translate(%f,%f)', $documentWidth, $documentHeight
                )
            );
...
sub process_rule {
    ...
    if ( ( $rule_layer ne '' ) && defined($layer) ) {
        if ( $rule_layer != $layer ) {
            debug( "rule has layer '$rule_layer', ignored on layer '$layer': "
                  . $rulenode->toString(1) )
              if ( $debug->{"rules"} );
            return;
        }
    ...
    if ( $rulenode->getName() eq "rule" ) {
        ...
        # normal selection
        $selection->[ $depth + 1 ] =
          make_selection( $rulenode, $selection->[$depth] );
        if ( $debug->{'rules'} ) {
            debug(  'rule "'
                  . $rulenode->toString(1)
                  . '" matches '
                  . $selection->[ $depth + 1 ]->size()
                  . ' elements' );
        }
    }
        ...
        $selection->[ $depth + 1 ] =
          $selection->[$depth] - $selection->[ $depth + 1 ];
        if ( $debug->{'rules'} ) {
            debug(  '"else" branch of rule "'
                  . $previous->toString(1)
                  . '" matches '
                  . $selection->[ $depth + 1 ]->size()
                  . ' elements' );
    ...
    foreach my $instruction ( $rulenode->getChildNodes() ) {
        ...
        if ( $name eq "layer" ) {
            ...
            if ( !defined($previous_child)
                || $previous_child->getName() ne "rule" )
            {
                debug(  "<else> not following <rule>, ignored: "
                      . substr( $instruction->toString(0), 0, 60 )
                      . "..." );
            }
            else {
...
sub make_selection {
    ...
    foreach ( keys(%$e_pieces) ) {
        warn(   'ignored invalid value "' 
              . $_
              . '" for e attribute in rule '
              . $rulenode->toString(1) )
    ...
    if ( $k eq '*' or !defined($k) ) {
        ...
        if ( $v eq "*" ) {
            ...
            # values is "~"
            $interim =
              select_elements_with_given_tag_key_and_value_fast( $oldsel,
                $e, $k, $v );
        }
            ...
            $interim =
              select_nodes_with_given_tag_key_and_value_for_way_fast( $oldsel,
                $k, $v );
        }
        else {
            ...
            $interim =
              select_elements_with_given_tag_key_and_value_slow( $oldsel,
                $e, $k, $v, $s );
        }
    }
...
# yielding reduced spacings the closer you get to the poles.
    ...
    if ( $notConnectedSameTag ne '' ) {
        $interim =
          select_not_connected_same_tag( $interim, $notConnectedSameTag );
    }
...
sub project {
    ...
    return [
        ...
        # new (proper merc.)
        $height +
          ( ProjectF($minlat) - ProjectF( $latlon->[0] ) ) * 180 /
          pi * 10000 *
          $scale
    ];
}
