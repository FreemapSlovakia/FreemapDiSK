...
our (
    $writer,          $project, $projection,  $symbolScale,
    $textAttenuation, $debug,   $meter2pixel, $text_index,
    %symbols,         $labelRelations
);
...
sub draw_lines {
    ...
    foreach (@$selected) {
        next
          unless ( ref $_ eq 'way' )
          ;    # Draw lines doesn't care about multipolygons
        ...
        $writer->startTag(
            "g",
            "class" => $class,
            ( $mask_class ne "" ) ? ( "mask-class", $mask_class ) : ()
        ) unless $group_started;
        $group_started = 1;
        if ($smart_linecaps) {
...
sub draw_way_with_smart_linecaps {
    ...
    if ( $first_node_connection_count > 1 ) {
        ...
        foreach my $otherway ( @{ $nodes->[0]->{"ways"} } ) {
            $first_node_lower_layer_connection_count++
              if ( $otherway->{'layer'} < $way->{'layer'} );
        }
    }

    my $last_node_connection_count =
      scalar( @{ $nodes->[ scalar(@$nodes) - 1 ]->{"ways"} } );
    ...
    if ( $last_node_connection_count > 1 ) {
        ...
        {
            $last_node_lower_layer_connection_count++
              if ( $otherway->{'layer'} < $way->{'layer'} );
        }
    }
    ...
    foreach (@$selected) {
        ...
        if ( $text ne '' ) {
            ...
            if ( ref $_ eq 'node' ) {
                ...
                debug( "Unhandled type in draw_text: " . ref($_) )
                  if ( $debug->{"drawing"} );
            }
        }
    }
...
sub draw_text_on_path {
    ...
    if ( $textnode->getAttribute("avoid-duplicates") =~ /^1|yes|true$/ ) {
        my $bucket1 =
          ( int( $nodes->[0]->{'lat'} * 2 ) + 180 ) * 720 +
          int( $nodes->[0]->{'lon'} * 2 ) + 360;
        my $bucket2 =
          ( int( $nodes->[ scalar @$nodes - 1 ]->{'lat'} * 2 ) + 180 ) * 720 +
          int( $nodes->[ scalar @$nodes - 1 ]->{'lon'} * 2 ) + 360;
        ...
        foreach my $label ( @{ $text_index->{$bucket} } ) {
            if ( $text eq $label->{"text"} ) {
                ...
                my $d2 =
                  distance( $nodes->[ scalar @$nodes - 1 ], $label->{'n1'} );
                ...
                $d2 =
                  distance( $nodes->[ scalar @$nodes - 1 ], $label->{'n0'} );
                debug("   distance to other: $d1 $d2")
                  if ( $debug->{"drawing"} );
                if ( $d1 < $threshold && $d2 < $threshold ) {
                    debug("ignore '$text'") if ( $debug->{"drawing"} );
    ...
    my $reverse =
      ( $nodes->[ scalar @$nodes - 1 ]->{"lon"} < $nodes->[0]->{"lon"} );
    ...
    if ($fontsize) {
        ...
        push(
            @{ $text_index->{$bucket} },
            {
                'text' => $text,
                'n0'   => $nodes->[0],
                'n1'   => $nodes->[ scalar @$nodes - 1 ]
            }
        ) if defined($bucket);
        my $extended = ($overflow) ? "_extended" : "";
        my $path =
          get_way_href( $id,
            ($reverse) ? 'reverse' . $extended : 'normal' . $extended );
        $writer->startTag(
            "text",
            copy_attributes_not_in_list(
                $textnode,
...
sub draw_area_text {
    ...
    foreach (@$selected) {
        ...
        if ( ref $_ eq 'way' or ref $_ eq 'multipolygon' ) {
            ...
            debug( "Unhandled type in draw_area_text: " . ref($_) )
              if ( $debug->{"drawing"} );
        }
    }
}
...
sub draw_symbols {
    ...
    foreach (@$selected) {
        ...
        if ( ref $_ eq 'way' or ref $_ eq 'multipolygon' ) {
            ...
            if ( defined($labelRelation) ) {
                foreach my $ref ( @{$labelRelation} ) {
                    draw_symbol( $symbolnode,
                        project [ $ref->{'lat'}, $ref->{'lon'} ] );
                }
            }
            else {
            ...
            debug( "Unhandled type in draw_symbols: " . ref($_) )
              if ( $debug->{"drawing"} );
        }
    }
}
...
sub draw_circles {
    ...
    foreach (@$selected) {
        ...
        if ( ref $_ eq 'way' or ref $_ eq 'multipolygon' ) {
            ...
            debug( "Unhandled type in draw_circles: " . ref($_) )
              if ( $debug->{"drawing"} );
        }
    }
}
...
sub draw_symbol {
    ...
    if ( $ref ne "" ) {
        ...
        $width = $symbolnode->getAttribute('width')
          unless $symbolnode->getAttribute('width') eq '';
        $height = $symbolnode->getAttribute('height')
          unless $symbolnode->getAttribute('height') eq '';
        ...
        $writer->startTag(
            "g",
            "transform" => sprintf(
                "translate(%f,%f) scale(%f) %s %s",
                $coordinates->[0], $coordinates->[1],
                $symbolScale,      $symbolnode->getAttribute("transform"),
                $shift
            )
        );

        my %copiedAttributes = copy_attributes_not_in_list(
            $symbolnode,
            [
        ...
        $writer->emptyTag(
            "use",
            copy_attributes_not_in_list(
                $symbolnode, [ "type", "scale", "smart-linecap" ]
            )
        );
        $writer->endTag("g");
    }
