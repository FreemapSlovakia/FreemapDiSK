...
sub select_elements_without_tags {
    ...
    foreach ( $oldsel->members() ) {
        next
          if defined($e)
              and ref($_) ne $e
              and not( $e eq 'way' and ref($_) eq 'multipolygon' );
...
sub select_elements_with_given_tag_key_and_value_fast {
    ...
    foreach my $key ( split( '\|', $k ) ) {
        ...
        debug(
            sprintf(
                '%d objects retrieved from index for e="%s" k="%s"',
                scalar(@objects), $e, $k
            )
        ) if ( $debug->{"indexes"} );
        ...
        foreach my $key (@keys_wanted) {
            foreach my $value (@values_wanted) {
                foreach my $tagset (@$tagsets) {
                    my $keyval = $tagset->{$k};
                    if (   ( $value eq '~' and !defined($keyval) )
                        or ( defined($keyval) and ( $value eq $keyval ) ) )
                    {
                        $newsel->insert($_);
                        next outer;
                    }
...
sub select_proximity {
    ...
    foreach ( $oldsel->members() ) {
        ...
        foreach my $ub ( @{ $used_boxes->{$pc} } ) {
            if (
                (
                       ( $ub->[0] > $bottom && $ub->[0] < $top )
                    || ( $ub->[2] > $bottom  && $ub->[2] < $top )
                    || ( $ub->[0] <= $bottom && $ub->[2] >= $top )
                )
                && (   ( $ub->[1] > $left && $ub->[1] < $right )
                    || ( $ub->[3] > $left  && $ub->[3] < $right )
                    || ( $ub->[1] <= $left && $ub->[3] >= $right ) )
              )
            {
...
sub select_minsize {
    ...
    foreach my $element ( $oldsel->members() ) {
        ...
        foreach my $node ( @{ $element->{"nodes"} } ) {
            $minlat = $node->{"lat"}
              if ( !defined($minlat) or $node->{"lat"} < $minlat );
            $minlon = $node->{"lon"}
              if ( !defined($minlon) or $node->{"lon"} < $minlon );
            $maxlat = $node->{"lat"}
              if ( !defined($maxlat) or $node->{"lat"} > $maxlat );
            $maxlon = $node->{"lon"}
              if ( !defined($maxlon) or $node->{"lon"} > $maxlon );
        }
        ...
        my $cirfer =
          ( $maxlat - $minlat ) +
          ( ( $maxlon - $minlon ) * ( 1.05 - ( ( $maxlat - 5 ) / 90 ) ) );
        $newsel->insert($element) if ( $cirfer > $minsize );

        debug(
            sprintf(
                'select_minsize: way=%s size=%f %s minsize %f',
                $element->{id}, $cirfer,
                $cirfer > $minsize ? 'larger than' : 'smaller than', $minsize
            )
        ) if ( $debug->{"selectors"} );
    }
...
sub select_closed {
    ...
    foreach my $element ( $selection->members() ) {
        ...
        if (
            (
                !defined( $element->{'tags'}->{'area'} )
                || $element->{'tags'}->{'area'} ne 'no'
            )
