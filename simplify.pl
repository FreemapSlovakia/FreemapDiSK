...
sub member {
    ...
    if ($type eq "way") {
        return if (!defined($OSM_WAYS->{$id}));
    }
    
    if ($type eq "node") {
        return if (!defined($OSM_NODES->{$id}));

        # If a duplicate node, locate the unique node ID.
        my $node_id = $OSM_NODES->{$id}->{id};

        for my $exist_id (@{$OSM_OBJ->{ref}}) {
    	    return if ($exist_id == $node_id);
        }
    }
...
sub nd {
    ...
    if (($OSM_OBJ->{element}) eq "way")
    {
        ...
        if ($previous_nd ne $node_id) {
            push(@{$OSM_OBJ->{nd}},$node_id);
            $previous_nd =$node_id;
        } 
    }
}
