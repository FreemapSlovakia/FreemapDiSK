...
die "Usage: $0 <layer> [<layer>]" if not $layers[0];
...
foreach my $layer (@layers) {
    my $min_zoom = $Config{"Layer.${layer}.MinZoom"};
    my $max_zoom = $Config{"Layer.${layer}.MaxZoom"};

    for (my $zoom = $min_zoom; $zoom <= $max_zoom; $zoom++) {
        my $rules_file = $Config{"Layer.${layer}.Rules.$zoom"};
...
sub output_wikka
{
    ...
    foreach my $color (sort_colors()) {
        $i++;
        print "|| $i ||{background:$color;} $color || $color " . ((defined($color_table_reverse{$color})) ? " (" . $color_table_reverse{$color} . ")" : "") . " || " . $colors{$color} . "||\n";
