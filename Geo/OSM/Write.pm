...
sub write_osm_file($$) { # Write an osm File
    ...
    my $osm_relations     = $osm->{relations};

    $osm->{tool} ||= "OSM-Tool";
    my $count_nodes    = 0;
    my $count_ways     = 0;
    my $count_relations     = 0;
	...
	if ( defined ( $bounds->{minlat} )) {
	# -90,-180,90,180
 	#<bounds minlat="48.9224992637582" minlon="22.1484375" maxlat="48.980216985375" maxlon="22.236328125" origin="http://www.freemap.sk/api/0.5"/>
	print $fh "  <bounds minlat=\"$bounds->{minlat}\" minlon=\"$bounds->{minlon}\" maxlat=\"$bounds->{maxlat}\" maxlon=\"$bounds->{maxlon}\" />\n";

    } else {
        print $fh "<!--calculated bouds -->\n";
    	my $minlat;
	    my $maxlat;
		my $minlon;
	    my $maxlon;

	    for my $node_id (  sort {$a <=> $b} keys %{$osm_nodes} ) {
		next unless $node_id;
		my $node = $osm_nodes->{$node_id};
		$minlat = $node->{lat} unless ( defined($minlat));
	    $maxlat = $node->{lat} unless ( defined($maxlat));
		$minlon = $node->{lon} unless ( defined($minlon));
	    $maxlon = $node->{lon} unless ( defined($maxlon));

		if ($minlat > $node->{lat}) { $minlat = $node->{lat}; };
        if ($maxlat < $node->{lat}) { $maxlat = $node->{lat}; };
		if ($minlon > $node->{lon}) { $minlon = $node->{lon}; };
        if ($maxlon < $node->{lon}) { $maxlon = $node->{lon}; };
		}
	print $fh "  <bounds minlat=\"$minlat\" minlon=\"$minlon\" maxlat=\"$maxlat\" maxlon=\"$maxlon\" />\n";
	}}
	...
	next unless scalar( @{$relation->{member}} )>0;

	print $fh "  <relation id=\'$relation_id\'>\n";
	for my $relation_member ( @{$relation->{member}} ) {
	    next unless $relation_member;
        my $ref =xml_encode($relation_member->{ref});
        my $role =xml_encode($relation_member->{role});
        my $type =xml_encode($relation_member->{type});
        
    
	    print $fh "    <member type=\'$type\' ref=\'$ref\' role=\'$role\' />\n";
	}

    print $fh tags2osm($relation);
    print $fh "  </relation>\n";
    $count_relations++;
    }
	...
	printf STDERR " Wrote OSM File ".
	    "($count_nodes Nodes, $count_ways Ways, , $count_relations Relations)";
