...
sub ReadExcludedTilesList {
	my %excluded;
	my $Filename = shift;
	my $lineNo = 0;

	if ( -f $Filename ) {
		open( my $fp, "<$Filename" ) || return %excluded;
		while ( my $line = <$fp> ) {
			chomp( $line );
			$lineNo++;
			next if ($line =~ /^\s*$/);	# only white spaces on the line
			next if ($line =~ /^\s*#.*$/);	# comment on the whole line
			if ( $line =~ /^\s*(\d{4})\s*(\d{4})\s*#?.*$/ ) { # tile specified by X and Y
				$excluded{"$1|$2"} = 1;
			} else {
				print STDERR "Wrong line #$lineNo: \"$line\"\n";
			}
		}
		close( $fp );
	}
	my $cnt = keys %excluded;
	print STDERR "Number of excluded tiles: $cnt\n";
	return %excluded;
}

sub GetTileExcludedTime {
	my ($tile_x, $tile_y, $exclTiles_ref) = @_;
	if (defined $$exclTiles_ref{"$tile_x|$tile_y"}) { return $$exclTiles_ref{"$tile_x|$tile_y"}; };
	return 0;
}

sub SetTileExcludedTime {
	my ($tile_x, $tile_y, $exclTiles_ref) = @_;
	return $$exclTiles_ref{"$tile_x|$tile_y"} = time();
}
